 ...
 *
 * The dispatcher of all task.
 *
 * Tasks are dispatched to here to be scheduled to execute sometimes in the future.
 */
public class HttpTaskDispatcher {
    private static ThreadPoolExecutor executor;
    private static BlockingQueue<Runnable> queue;

    private static HttpTaskDispatcher worker;

    private HttpTaskDispatcher() {
        super();
    }

    public static synchronized HttpTaskDispatcher getInstance() {
        if (worker == null) {
            worker = new HttpTaskDispatcher();
            doInit();
        }
        return worker;
    }
